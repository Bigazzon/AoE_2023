...
def setup(year, day):
    ...
    p = os.path.dirname(os.path.realpath(__file__)) + "/secret_session_cookie"
    if REQUESTS and os.path.isfile(p):
        with open(p) as f:
            SESSION = f.read().rstrip()
            S.cookies.set('session', SESSION)
    else:
        print(f'No secret_session_cookie file found at {p}')
...
# CACHE_DIR = os.path.join(UTILS_DIR, '..', 'inputs')
CACHE_DIR = os.path.join(Path(sys.argv[0]).parent)
YEAR = -1
DAY = -1
